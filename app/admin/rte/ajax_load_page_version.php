...
if($rs->RecordCount() == 0) {
	...
	foreach($a_pv as $pv) {
		$version_id = $pv['id'];
		$version = '(' . date_decode($pv['date_inserted'], $_SESSION['user_timezone'], $_SESSION['user_dateformat']) . ') ' .
			gettext('Submitted from') . ' ' . $pv['author_fullname'] . ' ' .
			gettext('as') . ' ' . $a_content_status[$pv['lk_content_status_id']] . ' ' .
			gettext('to be published from') . ' ' . date_decode($pv['date_publish_start'], $_SESSION['user_timezone'], $_SESSION['user_dateformat']) . ' ' .
			gettext('until') . ' ' . date_decode($pv['date_publish_end'], $_SESSION['user_timezone'], $_SESSION['user_dateformat']) . ' ' .
